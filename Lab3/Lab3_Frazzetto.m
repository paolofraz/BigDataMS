...
voltage = 10;                       % Motor power supply from 1 V to 15 V
unbalancing = 30;                  % Motor unbalancing weight from 0 g to 100 g
health = 70;                       % Motor state of health from 0% to 100%
...
spectrums = [];
 ...
 for v = 1:12
     ...
     [val, idx] = max(SSP(10:end)); % Look for peak avoiding the continuos component
...
spectrums = [];
 ...
 for w = 0:10:100
     ...
     unbalancing = w;                  % Motor unbalancing weight from 0 g to 100 g
     health = 100;                       % Motor state of health from 0% to 100%
     ...
     [val, idx] = max(SSP(f<=100 & f>1)); % Look for peak at <10kHz
     max_vibration = [max_vibration val];
     at_freq = [at_freq f(idx)];
 end
w = 0:10:100;
...
figure(4)
subplot(2,1,1)
plot(0:10:100,max_vibration)
subplot(2,1,2)
plot(w,at_freq)

p_w = polyfit(w,max_vibration,1);
%% Health
max_vibration = [];
at_freq = [];
signals = [];
...
subplot(2,1,2)
plot(h,at_freq)
mean_vib = mean(max_vibration(1:find(h==75),:),2); % take average for health <= 75%
p_h = polyfit(h(1:find(h==75)),mean_vib.',1);
